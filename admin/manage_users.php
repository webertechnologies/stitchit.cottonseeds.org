...
if(isset($_POST['submit'])){
    ...
    if(isset($_GET['id']) && $_GET['id']!=''){
        // update users
        mysqli_query($con, "UPDATE `users` SET `name`='$username',`email`='$email',`address`='$address',`pass`='$password' WHERE `users`.`id`=$id");
    }else{
        // insert new users 
        mysqli_query($con,"INSERT INTO `users` ( `id`,`name`,`email`,`address`,`pass`, `status` ) VALUES (NULL,'$username', '$email', '$address', '$password', '1')");
    }
    echo "<script>window.location.href='users.php'</script>";
    die();
}
                    ...
                    <div class="card">
                        ...
                        <div class="card-block">
                            <div class="card-body">
                                ...
                                <fieldset class="form-group">
                                    <input type="text" class="form-control" id="placeholderInput"
                                        value="<?php echo $password; ?>" name="password" placeholder="Enter password"
                                        required>
                                </fieldset>
